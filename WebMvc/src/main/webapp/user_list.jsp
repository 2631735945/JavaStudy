...
<script>
    layui.use('table', function(){
        ...
        table.render({
            elem: '#test'
            ,url:'${pageContext.request.contextPath}/layUser?method=selectByPage'
            ,toolbar: '#toolbarDemo'
        ...
        table.on('tool(layFilter)', function(obj){ // 双击 toolDouble
            ...
            if(obj.event === 'del'){
                layer.confirm('真的要删除该数据吗？', function(index){
                    $.post(
                        '${pageContext.request.contextPath}/layUser?method=deleteById',
                        {"id":data.id},
                        function (JSONResult){
            ...
            switch(obj.event){
                ...
                case 'deleteAll':
                    ...
                    layer.confirm('真的删除行么', function(index){
                        $.post(
                            '${pageContext.request.contextPath}/layUser?method=deleteAll',
                            {'ids' : ids},
                            function(jsonObj) {
                                console.log(jsonObj);
                                if (jsonObj.code == 0) {
